...
{
	/*
	 *  素直な条件式は almval + incr <= maxval2 であるが，この条件式
	 *  では，almval + incr が TickType で表せる範囲を超える場合に
	 *  正しく判定できなくなるため，次の条件式としている．
	 */
	if (incr <= (maxval2 - almval)) {
		...
		/*
		 *  素直な計算式は almval + incr - (maxval2 + 1) であるが，
		 *  この計算式では，almval + incr が TickType で表せる範囲を
		 *  超えるため正しく表現できない．また， maxval2 が TickType
		 *  で表せる範囲ちょうどの場合 maxval2 + 1 は正しく表現できない．
		 *  このため，オーバーフローを考慮すると次の計算式なる．
		 *  almval - (maxval2 - incr) - 1
		 *  しかし, オーバフローするだけで結果的に値は同じになるため、
		 *  ここでは素直な計算式で行う.
		 */
...
SignalCounter(CounterType cntid)
{
	...
	/*
	 *  newval == tick1val になるまで，1tick ずつカウンタを進めて
	 *  アラームの expire 判定を実施する．
	 *
	 *  1tick ずつ進めるのはアラームコールバック内で SetRelAlarm を
	 *  実行された場合に，正確な相対値を求める必要があるためである．
	 *
	 *  do ～ while にすれば以下の tick1val 初期化処理は不要．
	 */
	tick1val = cntcb_curval[cntid];
	while (newval != tick1val) {

		/*
		 *  カウンタの現在値を 1tick 更新
		 */
		tick1val = add_tick(cntcb_curval[cntid], 1, cntinib_maxval2[cntid]);
		cntcb_curval[cntid] = tick1val;

		/*
		 *  アラームの expire 処理
		 */
		while (((almid = cntcb_almque[cntid]) != ALMID_NULL)
					&& (almcb_almval[almid] == tick1val)) {
			/*
